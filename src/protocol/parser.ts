...
export class RESPParser {
  ...
  private offset: number = 0; // Keep track of the current position in the buffer
        ...
        if (Array.isArray(value)) {
          ...
          const command = value.map(item => {
            if (Buffer.isBuffer(item)) {
              ...
              console.error("Malformed command: array contains non-string/buffer element", item);
    ...
    switch (typeByte) {
      ...
      default:
        ...
        if (typeof typeByte !== 'number') {
             ...
             throw new Error(`Invalid state: typeByte is not a number at offset ${this.offset}`);
        }
        // Invalid type byte, throw error *without* incrementing offset
        // typeByte is guaranteed number here due to initial bounds check
        throw new Error(`Invalid RESP type byte: ${String.fromCharCode(typeByte)} (Code: ${typeByte}) at offset ${this.offset}`);
    }
  ...
  private _parseBulkString(): Buffer | null | undefined {
    ...
    const length = parseInt(line.toString('utf-8'), 10);
    if (isNaN(length)) {
    ...
    if (!this.buffer.subarray(this.offset + length, this.offset + totalLength).equals(CRLF)) {
        throw new Error("Malformed bulk string: Missing or incorrect trailing CRLF");
    }
  ...
  private _parseArray(): RESPValue[] | null | undefined {
    ...
    const count = parseInt(line.toString('utf-8'), 10);
     if (isNaN(count)) {
