...
import { formatError, formatBulkString } from "./index";
...
export function handleConfig(
    ...
    if (args.length < 1) {
        socket.write(formatError("ERR wrong number of arguments for 'config' command"));
        return;
    }
    ...
    if (subCommand === "GET") {
        if (args.length !== 2) {
            socket.write(formatError("ERR wrong number of arguments for 'config|get' command"));
            return;
        }
        ...
        // Not implemented yet
        socket.write(formatError("ERR Unsupported CONFIG subcommand or wrong number of arguments for 'SET'"));
    } else {
        socket.write(formatError(`ERR Unknown subcommand or wrong number of arguments for '${args[0]}'`));
    }
} 